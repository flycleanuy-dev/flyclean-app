...
| `Método de trabajo` | **multi_select** (`🚁 Dron` / `💪 Manual`) | Cómo se ejecutó el trabajo; se pueden marcar **ambos**. Obligatorio para registrar `Hora Inicio Efectivo`. (Era `select` hasta 2026-07-12) |
| `Herramienta manual` | **multi_select** (`Lanzas` / `Manguera` / `Hidrolavadora` / `Otro`) | Submétodo(s) cuando `Método de trabajo` incluye `💪 Manual`; **varias a la vez**; vacío si solo dron. (Era `select` hasta 2026-07-12) |
...
| `Registro jornadas` | rich_text (JSON) | Parte por día (Fase 3): `[{fecha,ini,fin,hechos:[ids]}]` — read-append-write. Al cerrar un servicio con sectores sin terminar, el operario elige **seguir otro día** (Estado→`🔄 Asignado`, `Fecha programada`→mañana, horas efectivas limpias) o **cerrar así** (→`✅ Completado`). "Completado del todo" cuando todos los sectores están `hecho`. (2026-06-30) |
