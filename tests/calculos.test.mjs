...
ok(C.montoOf({ 'Moneda': { select: { name: '🇺🇾 UY$' } }, 'Monto UY$': { number: 1000 } }, 'gasto').esUY === true, 'montoOf: etiqueta UY$ → pesos');
ok(C.montoOf({ 'Monto USD': { number: 50 } }, 'ingreso').esUY === false, 'montoOf: sin etiqueta + solo USD → dólares (legacy)');
ok(C.montoOf({ 'Monto UY$': { number: 800 } }, 'gasto').esUY === true, 'montoOf: sin etiqueta + solo UY$ → pesos (legacy)');
ok(C.montoOf({}, 'gasto').monto === 0, 'montoOf: sin datos → 0');
ok(C.montoOf({ 'Moneda cobro': { select: { name: '🇺🇸 USD' } }, 'Monto USD': { number: 200 } }, 'ingreso').monto === 200, 'montoOf: ingreso lee "Moneda cobro"');
...
const rows = [
  ...
  { properties: { 'Moneda': { select: { name: '🇺🇾 UY$' } }, 'Monto UY$': { number: 500 }, 'Excluir de KPIs': { checkbox: true } } },
];
...
ok(C.kpiIncluido({ properties: { 'Financiamiento': { select: { name: 'Neidat' } } } }) === false, 'kpiIncluido: financiamiento excluido');
ok(C.kpiIncluido({ properties: { '🗄️ Archivado': { checkbox: true } } }) === false, 'kpiIncluido: archivado excluido');
ok(C.kpiIncluido({ properties: { 'Tipo interno': { select: { name: '💱 Cambio a pesos' } } } }) === false, 'kpiIncluido: movimiento interno excluido');

// fmtTotalSplit — omite la línea en 0, nunca deja vacío
ok(C.fmtTotalSplit({ uyu: 100, usd: 0 }).includes('UY$') && !C.fmtTotalSplit({ uyu: 100, usd: 0 }).includes('USD'), 'fmtTotalSplit: omite la moneda en 0');
...
ok(C.tipoServicioStr({ 'Tipo de servicio': { multi_select: [{ name: '🏢 Fachada' }, { name: '🪟 Vidrios' }] } }) === '🏢 Fachada + 🪟 Vidrios', 'tipoServicioStr: multi_select');
ok(C.tipoServicioStr({ 'Tipo de servicio': { select: { name: '🏢 Fachada' } } }) === '🏢 Fachada', 'tipoServicioStr: select legacy');
