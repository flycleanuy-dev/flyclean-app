...
const PROSPECTO_MOCK = [
  { id: 'demo-1', properties: {
    ...
    'Notas prospección': { rich_text: [{ plain_text: 'Pidió cotización para 12 pisos, esperando respuesta.' }] }
  }},
  { id: 'demo-2', properties: {
...
const CLIENTE_MANT = { id: 'cli-mant', _mantMeses: 10, properties: {
...
async function login(page, country, userName, pin) {
  ...
  if (await consentBtn.isVisible().catch(() => false)) { await consentBtn.click(); await page.waitForTimeout(300); }
...
async function buildVentasSections(page) {
  ...
  sections.push({
    ...
    intro: 'Entrás con tu PIN y caés directo en 🎯 Prospección. Arriba tenés tus 4 pestañas: 🎯 Prospección · 💼 Propuestas · 👥 Clientes · 🗺️ Mapa. No ves finanzas ni la operativa: tu app es 100% comercial.',
    steps: [{
      title: '🎯 Prospección · 💼 Propuestas · 👥 Clientes · 🗺️ Mapa',
      description: 'Prospección es donde trabajás los leads nuevos. Propuestas y Clientes son para SEGUIR y RECONTACTAR (mirás, no editás). El Mapa te muestra objetivos para salir a buscar.',
      image: imgHome, wide: true
    }]
  });
  ...
  if (await nuevoBtn.count() > 0) {
    await nuevoBtn.click(); await page.waitForTimeout(900);
  ...
  sections.push({
    ...
    intro: 'Apenas terminás un llamado o visita, cargá el lead. Solo el nombre de la empresa/edificio es obligatorio; el resto suma pero no frena.',
    steps: [{
      title: 'Empresa · contacto · tel · zona · origen · interés · nota',
      description: 'Origen (🧲 Vendedor / 🤝 Referido / 🌐 Web / 📞 Entrante / 🚶 Puerta fría) e Interés (🏢 Fachada / 🪟 Vidrios / ☀️ Paneles) son botones de un toque. "Próximo contacto" se pre-carga a 3 días.',
      image: imgSheet, wide: true
    }]
  });
  ...
  sections.push({
    ...
    steps: [{
      ...
      description: '💬 WhatsApp abre el chat con un mensaje listo. "Contactado hoy" reprograma +7 días. "Interesado" lo marca caliente → ahí el COORDINADOR arma la propuesta (vos no cotizás). "Descartar" lo manda al fondo (no se borra).',
      image: imgLista, wide: true,
  ...
  sections.push({
    ...
    intro: 'Acá ves las cotizaciones que armó el coordinador y en qué estado están. Vos NO editás ni cambiás precios: mirás y recontactás para que no se enfríen.',
    steps: [{
      title: '"📞 A contactar hoy" + 💬 WhatsApp + 📞 Contactado',
      description: 'Arriba aparece "A contactar hoy": las propuestas que hace 15+ días esperan respuesta (con "☠️ le quedan N días" antes de vencer). 💬 WhatsApp abre el chat; 📞 Contactado registra que llamaste (resetea el reloj). No tocás importes ni estados.',
      image: imgProp, wide: true
    }]
  });
  ...
  }, { all: [CLIENTE_MANT, ...CLIENTES_ACTIVOS], secciones: { mantenimiento: [CLIENTE_MANT], activa: CLIENTES_ACTIVOS, sinRespuesta: [], rechazados: [] } });
  const imgCli = await snap(page, { wait: 600, fullPage: true });
  sections.push({
    title: '👥 Clientes — recontactá la cartera',
    intro: 'Ves TODA la cartera de clientes (datos de contacto), sin la plata. Arriba, destacados en "🔁 Mantenimiento", los que hace 9+ meses no tienen un trabajo: esos son los de recontactar para vender de nuevo.',
    steps: [{
      title: '💬 WhatsApp (abre) · 📞 Contactado (lo marcás vos)',
      description: 'Regla de oro: 💬 WhatsApp SOLO abre el chat. 📞 Contactado se marca APARTE y a mano, y solo cuando de verdad hablaste o mandaste el mensaje (si no atienden, NO lo marques). Al marcarlo, ese cliente sale de la lista de recontactar por 60 días para que no lo llamen dos veces.',
      image: imgCli, wide: true,
...
async function main() {
  ...
  try {
    ...
    const html = buildManualHTML({ title: 'Manual de Ventas', subtitle: 'Prospección, propuestas y clientes — tu circuito comercial', sections });
