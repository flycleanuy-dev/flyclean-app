...
export default async function handler(req, res) {
  ...
  if (!adminIds().includes(session.id)) return res.status(403).json({ ok: false, error: 'solo un admin puede cambiar PINs de otros usuarios' });

  const { targetId, newPin } = req.body || {};
  if (typeof targetId !== 'string' || !targetId || targetId.length > 60) return res.status(400).json({ ok: false, error: 'targetId inválido' });
  if (typeof newPin !== 'string' || !/^(\d{4}|\d{6})$/.test(newPin)) return res.status(400).json({ ok: false, error: 'El PIN debe ser de 4 o 6 dígitos' });
