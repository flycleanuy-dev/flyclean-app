...
export default async function handler(req, res) {
  ...
  if (typeof id !== 'string' || !/^[a-z0-9-]{2,60}$/.test(id)) return res.status(400).json({ ok: false, error: 'id inválido' });
  ...
  try {
    if (hard === true) {
      ...
      const r = await fetch(`${SB_URL}/rest/v1/usuarios?id=eq.${encodeURIComponent(id)}`, { method: 'DELETE', headers: SB_HEADERS() });
      if (!r.ok && r.status !== 404) return sbFail(res, r, 'eliminar');
      try { await deleteUserPin(id); } catch (_) {}
