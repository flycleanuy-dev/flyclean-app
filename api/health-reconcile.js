...
export default async function handler(req, res) {
  ...
  for (const t of TABLES) {
    try {
      ...
      if (!notion || supabase == null) { perTable[t] = { notion, supabase, skipped: true }; continue; }
  ...
  if (drift.length && !quiet) {
    try {
      ...
      const rows = drift.map(d => `<li><b>${d.t}</b>: Notion ${d.notion} vs espejo ${d.supabase} (dif ${d.diff > 0 ? '+' : ''}${d.diff})</li>`).join('');
      const body = `<p>El espejo Supabase difiere de Notion más allá de la tolerancia en <b>${drift.length}</b> tabla(s):</p><ul>${rows}</ul>` +
      ...
      await sendEmail({ to, subject: `⚠️ FlyClean · Espejo desincronizado (${drift.length} tabla/s)`, html: emailLayout('Reconciliación del espejo', body) });
    } catch (e) { console.warn('[reconcile] alerta falló', String(e?.message || e).slice(0, 120)); }
  }
