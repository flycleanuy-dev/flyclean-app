...
export default async function handler(req, res) {
  ...
  if (!adminIds().includes(session.id)) return res.status(403).json({ ok: false, error: 'solo un admin puede gestionar usuarios' });
  ...
  if (typeof id !== 'string' || !/^[a-z0-9-]{2,60}$/.test(id)) return res.status(400).json({ ok: false, error: 'id inválido (minúsculas, números y guiones, 2–60)' });
  if (typeof nombre !== 'string' || !nombre.trim() || nombre.trim().length > 80) return res.status(400).json({ ok: false, error: 'nombre inválido' });
  if (/[<>]/.test(nombre)) return res.status(400).json({ ok: false, error: 'el nombre no puede contener < o >' });
  ...
  try {
    ...
    const r = await fetch(url, {
      ...
      body: JSON.stringify({ id, nombre: nombre.trim(), rol, pais, activo: act, updated_at: new Date().toISOString() }),
    });
    if (r.status === 409) return res.status(409).json({ ok: false, error: 'ya existe un usuario con ese id (elegí otro id)' });
    if (!r.ok) { const t = await r.text().catch(() => ''); console.warn('admin-set-user supabase', r.status, t.slice(0, 200)); return res.status(500).json({ ok: false, error: 'no se pudo guardar (supabase ' + r.status + ')' }); }
