...
export async function loadUsersFromDb() {
  ...
  const map = {};
  for (const x of rows) {
    if (x && x.id && x.activo !== false)
      map[x.id] = { nombre: x.nombre, rol: x.rol, pais: x.pais, emoji: x.emoji || null };
  }
  return map;
}
...
function refreshDbUsersIfStale() {
  ...
  loadUsersFromDb()
    ...
    .catch(e => {
      console.warn('[users] loadUsersFromDb falló → fallback hardcoded:', e.message);
    })
    .finally(() => {
      _dbUsersLoading = false;
    });
}
...
export async function resolveUser(id) {
  ...
  try {
    _dbUsers = await loadUsersFromDb();
    _dbUsersAt = Date.now();
  } catch (_) {
    return USERS[id] || null;
  } // rescate falló → fallback duro
...
export function esVentas(u) {
  return !!(u && String(u.rol || '').includes('Ventas'));
}
