...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (pending.missing) return res.status(200).json({ ok: true, note: 'outbox no provisionado', pending: 0, error: 0, oldest_pending_min: 0 });
    ...
    const or = await fetch(`${SUPABASE_URL}/rest/v1/outbox_notion?status=eq.pending&order=created_at.asc&limit=1&select=created_at`, { headers: sbHeaders() });
    const rows = await or.json().catch(() => []);
    if (Array.isArray(rows) && rows[0]?.created_at) oldest_pending_min = Math.round((Date.now() - new Date(rows[0].created_at).getTime()) / 60000);
    const healthy = (errored.count || 0) === 0 && oldest_pending_min < 30;
    return res.status(200).json({ ok: true, healthy, pending: pending.count || 0, error: errored.count || 0, oldest_pending_min });
  } catch (e) {
